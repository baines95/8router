    ...
    <div className={className}>
      ...
      {!hideTrigger && (
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 px-3 py-2 text-text-muted hover:text-text-main hover:bg-surface/60 transition-colors"
          title="Language"
          data-i18n-skip="true"
        >
      ...
      {isOpen && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" data-i18n-skip="true">
          ...
          <div
            ref={modalRef}
            className="relative w-full bg-surface border border-black/10 dark:border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 max-w-2xl flex flex-col max-h-[80vh]"
          >
            {/* Modal header */}
            <div className="flex items-center justify-between p-3 border-b border-black/5 dark:border-white/5">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="p-1.5 text-text-muted hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                aria-label="Close"
              >
                ...
                {LOCALES.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isPending}
                      className={`flex flex-col items-center justify-start gap-1 px-2 py-3 text-xs font-medium transition-colors w-full ${
                        active
