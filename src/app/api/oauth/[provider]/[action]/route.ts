...
export async function GET(request: Request, context: { params: Promise<{ provider: string, action: string }> }): Promise<NextResponse> {
  try {
    ...
    if (action === "device-code") {
      ...
      const authData = generateAuthData(provider, "");
...
export async function POST(request: Request, context: { params: Promise<{ provider: string, action: string }> }): Promise<NextResponse> {
  try {
    const { provider, action } = await context.params;
    let body: any;
    try {
