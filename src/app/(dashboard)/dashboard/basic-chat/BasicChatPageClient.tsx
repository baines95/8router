...
export default function BasicChatPageClient({ initialData }: BasicChatPageClientProps) {
 ...
 return Array.isArray(savedSessions) ? savedSessions.map((s: any) => ({ ...s, messages: Array.isArray(s.messages) ? s.messages : [] })) : [];
 });
 const [activeSessionId, setActiveSessionId] = useState(() => typeof window === "undefined" ? "" : (globalThis.localStorage.getItem(STORAGE_KEYS.activeSessionId) || ""));
  
 const [activeProviderId, setActiveProviderId] = useState(() => typeof window === "undefined" ? "" : (globalThis.localStorage.getItem(STORAGE_KEYS.activeProviderId) || ""));
 const [activeModelId, setActiveModelId] = useState("");
 const [draft, setDraft] = useState(() => typeof window === "undefined" ? "" : (globalThis.localStorage.getItem(STORAGE_KEYS.draft) || ""));
 ...
 {m.attachments.map(a => <Image key={a.id} src={a.dataUrl} alt={a.name} width={320} height={160} className="rounded-none border border-border/50 max-h-40 object-cover w-full shadow-none" unoptimized />)}
 </div>
 )}
