...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
 ...
 if (connection.isActive === false) return <Badge variant="secondary"className="h-4 text-xs font-medium capitalize opacity-50">Đã tắt</Badge>;
 if (effectiveStatus ==="active"|| effectiveStatus ==="success") return <Badge className="h-4 text-xs font-medium capitalize bg-primary/10 text-primary border-none">Hoạt động</Badge>;
 if (effectiveStatus ==="error"|| effectiveStatus ==="expired"|| effectiveStatus ==="unavailable") return <Badge variant="destructive"className="h-4 text-xs font-medium capitalize border-none">Lỗi</Badge>;
 return <Badge variant="outline"className="h-4 text-xs font-medium capitalize border-border/50">{effectiveStatus ||"Không rõ"}</Badge>;
 };
 ...
 <div className="size-8 rounded-lg bg-muted border border-border/50 flex items-center justify-center shrink-0">
 {isOAuth ? <Lock className="size-3.5 text-muted-foreground"/> : <Key className="size-3.5 text-muted-foreground"/>}
 </div>
 <div className="min-w-0 flex-1 space-y-1">
 <div className="flex items-center gap-2">
 <span className="text-sm font-medium truncate">{isOAuth ? (connection.name || connection.email ||"OAuth Account") : connection.name}</span>
 {getStatusBadge()}
 {hasAnyProxy && <Badge variant="secondary"className={cn("h-4 text-xs font-semibold uppercase border-none", boundProxyPool?.isActive ?"bg-primary/10 text-primary":"bg-muted/30 text-muted-foreground")}>PROXY</Badge>}
 <span className="text-xs font-medium text-muted-foreground/40 tabular-nums">#{connection.priority}</span>
 </div>
 <div className="flex items-center gap-2">
 ...
 <DropdownMenuLabel className="text-xs font-semibold uppercase tracking-tight text-muted-foreground/60">Route Via Proxy</DropdownMenuLabel>
 <DropdownMenuSeparator />
 ...
 <Button variant="ghost"size="icon"className="size-8 rounded-full text-muted-foreground hover:text-destructive"onClick={onDelete}><Trash2 className="size-3.5"/></Button>
 </div>
...
function AddApiKeyModal({ isOpen, provider, providerName, proxyPools, onSave, onClose }) {
 ...
 <Label className="text-xs font-medium uppercase tracking-tight text-muted-foreground">Label</Label>
 <Input value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Production Cluster"/>
 </div>
 <div className="space-y-2">
 <Label className="text-xs font-medium uppercase tracking-tight text-muted-foreground">API Token</Label>
 ...
 <Button variant="outline"size="sm"onClick={handleValidate} disabled={!formData.apiKey || validating} className="h-9 px-4 text-xs font-medium uppercase">{validating ? <RefreshCw className="size-3 animate-spin"/> :"Verify"}</Button>
 </div>
 {validationResult && <Badge variant={validationResult === 'success' ? 'secondary' : 'destructive'} className="h-4 text-xs font-semibold uppercase">{validationResult === 'success' ? 'TOKEN VALID' : 'TOKEN FAIL'}</Badge>}
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div className="grid gap-2">
 <Label className="text-xs font-medium uppercase tracking-tight text-muted-foreground">Priority</Label>
 <Input type="number"value={formData.priority} onChange={e => setFormData({ ...formData, priority: parseInt(e.target.value) || 1 })} />
 </div>
 <div className="grid gap-2">
 <Label className="text-xs font-medium uppercase tracking-tight text-muted-foreground">Proxy Hub</Label>
 ...
 <DialogFooter>
 <Button variant="ghost"className="font-medium text-xs uppercase tracking-tight"onClick={onClose}>Cancel</Button>
 <Button className="font-medium text-xs uppercase tracking-tight px-8"onClick={handleSubmit} disabled={saving || !formData.apiKey || !formData.name}>{saving ?"Saving...":"Connect"}</Button>
 </DialogFooter>
 </DialogContent>
 </Dialog>
 );
...
export default function ConnectionsCard({ providerId, isOAuth }) {
 ...
 <CardTitle className="text-xs font-semibold uppercase tracking-tight text-muted-foreground/70">Gateway Node Routing</CardTitle>
 <div className="flex items-center gap-4">
 <div className="flex items-center gap-2">
 <span className="text-xs font-semibold uppercase tracking-tight text-muted-foreground opacity-50">Round Robin</span>
 ...
 <div className="flex items-center gap-2">
 <span className="text-xs font-semibold uppercase tracking-tight text-muted-foreground opacity-50">Sticky</span>
 <Input type="number"min={1} value={providerStickyLimit} onChange={e => { setProviderStickyLimit(e.target.value); saveStrategy('round-robin', e.target.value); }} className="h-6 w-12 px-1 text-xs font-medium text-center"/>
 </div>
 )}
 </div>
 </CardHeader>
