...
export default function ProfilePageClient({ initialData }) {
 ...
 <div className="flex items-center justify-between p-3 rounded-lg bg-bg border border-border/50">
 <div>
 <p className="font-medium">Database Location</p>
 <p className="text-sm text-text-muted font-mono">~/.8router/db.json</p>
 </div>
 </div>
 ...
 {dbStatus.message && (
 <p className={`text-sm ${dbStatus.type ==="error"?"text-destructive":"text-primary dark:text-primary"}`}>
 {dbStatus.message}
 </p>
 )}
 </div>
 ...
 Setting password for the first time. Leave current password empty or use default: <code className="bg-primary/10 px-1 rounded">123456</code>
 </p>
 </div>
 )} */}
 ...
 {passStatus.message && (
 <p className={`text-sm ${passStatus.type ==="error"?"text-destructive":"text-primary"}`}>
 {passStatus.message}
 </p>
 )}
 ...
 {proxyStatus.message && (
 <p className={`text-sm ${proxyStatus.type ==="error"?"text-destructive":"text-primary"} pt-2 border-t border-border/50`}>
 {proxyStatus.message}
 </p>
 )}
 </div>
