...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      else { const data = await res.json(); setError(data.error || "Invalid credential identity."); }
    } catch (err) { setError("Node connection failure."); } finally { setLoading(false); }
  };
