...
export interface OAuthProviderHandler {
  ...
  fixedPort?: number;
  callbackPath?: string;
  buildAuthUrl?: (config: any, redirectUri: string, state: string, codeChallenge?: string, meta?: any) => string;
  exchangeToken?: (config: any, code: string, redirectUri: string, codeVerifier: string, state: string, meta?: any) => Promise<any>;
  requestDeviceCode?: (config: any, codeChallenge?: string, options?: any) => Promise<any>;
  pollToken?: (config: any, deviceCode: string, codeVerifier?: string, extraData?: any) => Promise<any>;
  postExchange?: (tokens: any) => Promise<any>;
  mapTokens: (tokens: any, extra?: any) => any;
}
 ...
 */
export async function exchangeTokens(providerName: string, code: string, redirectUri: string, codeVerifier: string, state: string, meta?: any) {
  const provider = getProvider(providerName);
 ...
 */
export async function requestDeviceCode(providerName: string, codeChallenge?: string, options?: any) {
  const provider = getProvider(providerName);
  if (provider.flowType !== "device_code") {
 ...
 */
export async function pollForToken(providerName: string, deviceCode: string, codeVerifier?: string, extraData?: any) {
  const provider = getProvider(providerName);
  if (provider.flowType !== "device_code") {
    throw new Error(`Provider ${providerName} does not support device code flow`);
  }
