...
export function hasValidContent(msg: any): boolean {
...
export function fixToolUseOrdering(messages: any[]): any[] {
  ...
  for (const msg of messages) {
    ...
    if (last && last.role === msg.role) {
      ...
      const toolResults = [...lastContent.filter((b: any) => b.type === "tool_result"), ...msgContent.filter((b: any) => b.type === "tool_result")];
      const otherContent = [...lastContent.filter((b: any) => b.type !== "tool_result"), ...msgContent.filter((b: any) => b.type !== "tool_result")];
