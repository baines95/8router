...
export class CursorExecutor extends BaseExecutor {
  ...
  async makeFetchRequest(url: string, headers: any, body: any, signal: AbortSignal | null, proxyOptions = null) {
  ...
  makeHttp2Request(url: string, headers: any, body: any, signal: AbortSignal | null): Promise<any> {
    if (!http2) {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.on("response", (hdrs: any) => { responseHeaders = hdrs; });
  ...
  transformProtobufToJSON(buffer: Buffer, model: string, body: any) {
    ...
    while (offset < buffer.length) {
      ...
      const decompressed = decompressPayload(payload, flags);
      if (!decompressed) {
        debugLog(`[CURSOR BUFFER] Frame ${frameCount}: decompression failed, skipping`);
        continue;
      }
      
      // Cast to Buffer to avoid ArrayBufferLike issue
      payload = Buffer.from(decompressed);
      ...
      const result = extractTextFromResponse(new Uint8Array(payload as any));
  ...
  transformProtobufToSSE(buffer: Buffer, model: string, body: any) {
    const responseId = `chatcmpl-cursor-${Date.now()}`;
    const created = Math.floor(Date.now() / 1000);
