...
// Parse SSE data line
export function parseSSELine(line: string, format: string | null = null): any {
  if (!line) return null;
...
export function hasValuableContent(chunk: any, format: string): boolean {
  // OpenAI format
...
export function fixInvalidId(parsed: any): boolean {
...
function cleanUsagePayload(payload: any): any {
  ...
  if ("usage" in cleaned) {
    if (cleaned.usage === null) {
      ...
      cleaned.usage = usageWithoutPerf;
    }
  }
...
export function formatSSE(data: any, sourceFormat: string): string {
  ...
  const cleanedData = cleanUsagePayload(data);

  // Claude format
  if (sourceFormat === FORMATS.CLAUDE && cleanedData && cleanedData.type) {
    return `event: ${cleanedData.type}\ndata: ${JSON.stringify(cleanedData)}\n\n`;
  }

  return `data: ${JSON.stringify(cleanedData)}\n\n`;
}
