 ...
 *  - proxyFetch.js: direct fetch behavior remains stable for Anthropic and non-Anthropic hosts
 */
...
// ─── proxyFetch direct fetch behavior ─────────────────────────────────────────

describe("proxyAwareFetch — direct fetch behavior", () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.resetModules();
    vi.doUnmock("undici");
  ...
  it("throws when strict proxy is enabled and the proxy request fails", async () => {
  ...
  it("uses the same direct fetch path for non-Anthropic hosts", async () => {
    const undiciFetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      statusText: "OK",
      headers: new Headers(),
